...
module osr_ram_cipher_ecc_top #(
    parameter  P_RAM_AW         = 17    ,
...
localparam     ECC_ENC_WIDTH_DOUT   = (!P_RAM_ECC_EN) ? ECC_ENC_WIDTH_DIN + 17 : ECC_ENC_WIDTH_DIN + ECC_ENC_WIDTH_CHKBIT;
...
if(P_RAM_CIPHER_EN)begin:cipher_enable
   ...
   wire [CIPHER_RAM_p_DATA_WIDTH-1:0]     cipher_i_cipher_rdata = !rd_flg_vail ? {{CIPHER_RAM_p_DATA_WIDTH}{1'b0}} : ecc_dec_decode_dout[P_RAM_DW+17-1:17];
   ...
   assign cipher_o_plain_rdata  = !rd_flg_vail ? {{CIPHER_RAM_p_DATA_WIDTH}{1'b0}} : ecc_dec_decode_dout[P_RAM_DW+17-1:17];
...
if(P_RAM_ECC_EN)begin:ecc_enable
    ...
    assign ecc_enc_encode_dout = |o_c_ram_wen ?  {cipher_o_cipher_wdata,{{{17-P_RAM_AW}{1'b0}},cipher_o_cipher_waddr}} : {(ECC_ENC_WIDTH_DIN+17){1'b0}}; 
    assign ecc_dec_ecc_sec     = 1'b0;
    assign ecc_dec_ecc_ded     = 1'b0;
    assign ecc_dec_decode_dout = rd_flg_vail ? {i_c_ram_rdata,17'h0} : {{ECC_DEC_WIDTH_DIN}{1'b0}};
end
endgenerate
...
assign o_c_ram_wdata       = i_p_ram_cs ? ecc_enc_encode_dout[ECC_ENC_WIDTH_DOUT-1:17] : {{P_RAM_ECC_DW}{1'b0}} ;
