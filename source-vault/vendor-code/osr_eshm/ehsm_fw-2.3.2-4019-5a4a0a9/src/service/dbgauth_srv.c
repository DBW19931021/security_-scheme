 ...
 **********************************************************************************************************************/
/*Definition the UID */
#define DBG_AUTH_OTP_UID_ADDRESS (CONFIG_EHSM_OTP_BASE_ADDR + 4U)

#define DEBUG_AUTH_CFI_INIT_VAL  1000
#define DEBUG_AUTH_CFI_FINAL_VAL 1006
/***********************************************************************************************************************
 *  TYPEDEFS
 **********************************************************************************************************************/
/*Definition the soc debug mode*/
typedef enum {
    SOC_DBG_PORT_OPEN_MODE = 0x00,
    SOC_DBG_PORT_CLOSE_MODE = 0x01,
} soc_dbg_switch_mode_e;
...
static bool_t g_user_auth_result = FALSE;
 ...
 **********************************************************************************************************************/
/**
 *   @brief      Close the soc debug port according to soc debug bitmap data
 *
 *   @param
 *
 *   @return     uint32_t
 ...
 *   @note
 */
static uint32_t dbgauth_switch_soc_debug_port(soc_dbg_switch_mode_e mode, const uint32_t soc_dbg_bitmap[5]);
/***********************************************************************************************************************
 *  LOCAL FUNCTION
 **********************************************************************************************************************/
/**
...
{
    static uint8_t random_data[8];
    static uint32_t cnt = 0;

    if ((cnt % 8U) == 0U) {
        (void)cpt_get_rand_fast(random_data, 8);
        cnt = 0;
    }

    // sleep random number & securely cmp
    uint32_sleep((uint32_t)random_data[cnt] & 0x1FU, (random_data[cnt] >> 4U));
    cnt++;
}
/**
 ...
 *   @param [out] challenge_data Output the address of local challenge data
 *
 *   @return     uint32_t
 *
 *   @note
...
static uint32_t check_lifecycle_type(uint8_t e_type, uint8_t **challenge_data)
{
    ...
    if (EHSM_ERR_SW_SUCCESS == ret) {
        ...
        if (MB_EHSM_CHALLENGE_TYPE_SOC_DEBUG == e_type) {
            *challenge_data = g_soc_challenge_buf;
        }


        if (MB_EHSM_CHALLENGE_TYPE_USER_AUTH == e_type) {
 ...
 *   @note
 */
static void set_challenge_st(uint8_t challenge_type, uint8_t *challenge_buf)
{
    if (MB_EHSM_CHALLENGE_TYPE_EHSM_DEBUG == challenge_type) {
...
/**
 *   @brief      Check debug authentication data of RSA
 *
 *   @param [in out] debug_auth_st A point pointer to struction of ehsm_debug_auth_st
 *
 *   @return     uint32_t
 *
 *   @note
 */
static uint32_t check_rsa_auth_data(ehsm_debug_auth_st *debug_auth_st)
{
    uint32_t ret = EHSM_ERR_SW_SUCCESS;
    ...
    if (debug_auth_st == NULL) {
        ...
        if (debug_auth_st->signature_size != (debug_auth_st->public_key_size - 64U)) {
            ret = EHSM_ERR_WRONG_DATA_LENGTH;
        } else if ((debug_auth_st->public_key_size == (64U + 256U))
            || (debug_auth_st->public_key_size == (64U + 384U))) {
            debug_auth_st->public_key = &debug_auth_st->public_key[4];
        } else {
            ret = EHSM_ERR_WRONG_DATA_LENGTH;
        }
    }
...
/**
 *   @brief      Check debug authentication data of SM2
 *
 *   @param [in out] debug_auth_st A point pointer to struction of ehsm_debug_auth_st
 *
 *   @return     uint32_t
 ...
 *   @note
 */
static uint32_t check_sm2_auth_data(ehsm_debug_auth_st *debug_auth_st)
{
    uint32_t ret = EHSM_ERR_SW_SUCCESS;
    ...
    if (debug_auth_st == NULL) {
        ...
        if (debug_auth_st->signature_size != 64U) {
            ret = EHSM_ERR_WRONG_DATA_LENGTH;
        } else if ((debug_auth_st->public_key_size == 65U) && (debug_auth_st->public_key[4] == 0x04U)) {
            debug_auth_st->public_key = &debug_auth_st->public_key[4];
        } else if (debug_auth_st->public_key_size == 64U) {
            // add prefix 0x04
            debug_auth_st->public_key = &debug_auth_st->public_key[3];
            debug_auth_st->public_key[0] = 0x04;
            debug_auth_st->public_key_size = 65U;
        } else {
            ret = EHSM_ERR_WRONG_DATA_LENGTH;
        }
    }
...
/**
 *   @brief      Check debug authentication data of ECC
 *
 *   @param [in out] debug_auth_st A point pointer to struction of ehsm_debug_auth_st
 *
 *   @return     uint32_t
 ...
 *   @note
 */
static uint32_t check_ecc_auth_data(ehsm_debug_auth_st *debug_auth_st)
{
    uint32_t ret = EHSM_ERR_SW_SUCCESS;
...
static uint32_t check_auth_data(
    uint8_t challenge_type, ehsm_debug_auth_st *debug_auth_st, ehsm_get_challenge_st **challenge)
{
    uint32_t ret = EHSM_ERR_SW_SUCCESS;

    if ((debug_auth_st == NULL) || (challenge == NULL)) {
        ret = EHSM_ERR_PARAM_ERROR;
    } else {
        switch (debug_auth_st->alg) {
        case MB_AUTH_ALG_SM2_WITH_SM3:
            ret = check_sm2_auth_data(debug_auth_st);
            break;
        case MB_AUTH_ALG_ECCSECP256R1_WITH_SHA256:
            ret = check_ecc_auth_data(debug_auth_st);
            break;
        case MB_AUTH_ALG_SHA256_RSA:
            // support RSA2048 and RSA3072
            ret = check_rsa_auth_data(debug_auth_st);
            break;

        case MB_AUTH_ALG_SM4_CMAC:
        case MB_AUTH_ALG_AES128_CMAC:
            if (debug_auth_st->signature_size != 16U) {
                ret = EHSM_ERR_WRONG_DATA_LENGTH;
            }
            break;
...
static uint32_t check_challenge_data(uint8_t type, const ehsm_get_challenge_st *challenge, uint32_t *otp_key_logic_id)
{
    ...
    if ((challenge == NULL) || (otp_key_logic_id == NULL)) {
        ...
        if (MB_EHSM_CHALLENGE_TYPE_USER_AUTH == type) {
            if ((challenge->size != EHSM_DEBUG_CHALLENGE_SIZE)
    ...
    uint8_t type, const ehsm_get_challenge_st *challenge, uint8_t *hash_value, uint8_t alg, uint16_t *sp_key_id)
{
    ...
    if (EHSM_ERR_SW_SUCCESS == ret) {
        if ((alg == MB_AUTH_ALG_SM4_CMAC) || (alg == MB_AUTH_ALG_AES128_CMAC)) {
            algo_type = OTP_KEY_ALGO_SKE_TYPE;
        } else {
            algo_type = OTP_KEY_ALGO_HASH_TYPE;
        }

        ret = otpkey_check_usage(otp_key_logic_id, algo_type, OTP_KEY_LEVEL_NO_CHECK, KEY_USAGE_NONE);
    ...
    const ehsm_debug_auth_st *debug_auth, const ehsm_get_challenge_st *challenge, const uint8_t *inner_hash)
{
    ...
    if ((debug_auth == NULL) || (challenge == NULL) || (inner_hash == NULL)) {
        ...
        fid_inc_u32(&g_debug_auth_cfi, 1);
        if (EHSM_ERR_SW_SUCCESS == ret) {
            if (util_memcmp(inner_hash, tmpHash, 32) != 0U) {
                ret = EHSM_ERR_DEBUG_AUTH_PK_HASH_MISMATCH;
            }
        }
    }
    ...
    const ehsm_debug_auth_st *debug_auth, const ehsm_get_challenge_st *challenge, const uint8_t *inner_hash)
{
    ...
    if ((debug_auth == NULL) || (challenge == NULL) || (inner_hash == NULL)) {
        ...
        fid_inc_u32(&g_debug_auth_cfi, 1);
        if (EHSM_ERR_SW_SUCCESS == ret) {
            if (0U != (util_memcmp(inner_hash, tmpHash, 32))) {
                ret = EHSM_ERR_DEBUG_AUTH_PK_HASH_MISMATCH;
            }
        }
    }
    ...
    if (EHSM_ERR_SW_SUCCESS == ret) {
        ...
        if (EHSM_ERR_SW_SUCCESS == ret) {
            ...
            ret = cpt_rsa_ssa_pss_verify_by_msg_digest(HASH_SHA256, HASH_SHA256, -1, tmpHash, debug_auth->public_key,
                RSA_PUBLIC_K_E_LEN * 8U, &debug_auth->public_key[RSA_PUBLIC_K_E_LEN],
                (debug_auth->public_key_size - RSA_PUBLIC_K_E_LEN) * 8U, debug_auth->signature);
            fid_inc_u32(&g_debug_auth_cfi, 1);
        }
    }
    ...
    if (EHSM_ERR_SW_SUCCESS == ret) {
        ...
        if (EHSM_ERR_SW_SUCCESS == ret) {
            ...
            ret = cpt_ecdsa_verify(
                (const cpt_eccp_curve_st *)secp256r1, tmpHash, 32, debug_auth->public_key, debug_auth->signature);
            fid_inc_u32(&g_debug_auth_cfi, 1);
            if (EHSM_ERR_SW_SUCCESS != ret) {
                ret = EHSM_ERR_DEBUG_AUTH_FAILED;
            }
 ...
 *   @return     uint32_t
 *
 *   @note Because some life cycle of firmware the ske key can not be read by CPU so SKE using key through secure port
 * index
 */
    ...
    const ehsm_debug_auth_st *debug_auth, const ehsm_get_challenge_st *challenge, uint16_t sp_key_id)
{
    ...
    if ((debug_auth == NULL) || (challenge == NULL)) {
        ...
        if (debug_auth->alg == MB_AUTH_ALG_SM4_CMAC) {
            alg = SKE_ALG_SM4;
        }

        if (debug_auth->alg == MB_AUTH_ALG_AES128_CMAC) {
            alg = SKE_ALG_AES;
        }
        ...
        fid_inc_u32(&g_debug_auth_cfi, 1);
        if (EHSM_ERR_SW_SUCCESS == ret) {
            if (util_memcmp(debug_auth->signature, cmac, EHSM_DBG_AUTH_CMAC_SIGNATURE_SIZE) != 0U) {
                ret = EHSM_ERR_DEBUG_AUTH_FAILED;
            }
        }
...
/**
 *   @brief      Check the life cycle whether is allowed to do debug authentication or close debug authentication result
 *
...
/**
 *   @brief      Get the challenge data handler
 *
 *   @param [in] req_data
 *   @param [in] rsp_data
 *
...
static uint32_t dbgauth_get_challenge_handle(const cmd_req_data_st *req_data, const cmd_rsp_data_st *rsp_data)
{
    ...
    uint32_t ret = EHSM_ERR_SW_SUCCESS;
    const mb_cmd_get_challenge_st *cmd_data = NULL;
    uint8_t challenge_buf[EHSM_DEBUG_CHALLENGE_SIZE];

    if ((req_data == NULL) || (rsp_data == NULL)) {
        ret = EHSM_ERR_PARAM_ERROR;
    } else {
        cmd_data = (const mb_cmd_get_challenge_st *)(req_data);
    ...
    if (EHSM_ERR_SW_SUCCESS == ret) {
        ...
        if (EHSM_ERR_SW_SUCCESS == ret) {
            {
                ret = otp_read(DBG_AUTH_OTP_UID_ADDRESS, &out_data[32], 16);
                if (EHSM_ERR_SW_SUCCESS == ret) {
                    util_memcpy(challenge_buf, out_data, EHSM_DEBUG_CHALLENGE_SIZE);
                }
            }

            set_challenge_st(cmd_data->type, out_data);
        }

        if (EHSM_ERR_SW_SUCCESS == ret) {
            {
                ret = mmap_write_remote_data(cmd_data->addr, challenge_buf, EHSM_DEBUG_CHALLENGE_SIZE);
            }
        } else {
...
{
    uint8_t pubkey[MAX_PUBKEY_SIZE + 4U];
    uint8_t signatures[MAX_SIGNATURE_SIZE];
    uint32_t ret = EHSM_ERR_SW_SUCCESS;
    ehsm_debug_auth_st ehsm_debug_auth[1];
    const mb_cmd_auth_st *cmd_data = NULL;

    if ((req_data == NULL) || (rsp_data == NULL)) {
        ret = EHSM_ERR_PARAM_ERROR;
    } else {
        cmd_data = (const mb_cmd_auth_st *)(req_data);
        if ((MB_EHSM_CHALLENGE_TYPE_EHSM_DEBUG == cmd_data->type)
            ...
            if (((cmd_data->alg == MB_AUTH_ALG_SM2_WITH_SM3) || (cmd_data->alg == MB_AUTH_ALG_ECCSECP256R1_WITH_SHA256)
                    || (cmd_data->alg == MB_AUTH_ALG_SHA256_RSA))
            ...
            } else if (((cmd_data->alg == MB_AUTH_ALG_SM4_CMAC) || (cmd_data->alg == MB_AUTH_ALG_AES128_CMAC))
    ...
    if ((EHSM_ERR_SW_SUCCESS == ret) && (MB_EHSM_CHALLENGE_TYPE_SOC_DEBUG == cmd_data->type)) {
        if (cmd_data->soc_dbg_bitmap_size != 5U) {
            ...
            ret = mmap_read_remote_data(
                ehsm_debug_auth->soc_dbg_bitmap, cmd_data->soc_dbg_bitmap_addr, 5U * sizeof(uint32_t));
        }
    }
...
static uint32_t dbgauth_close_debug_handle(const cmd_req_data_st *req_data, const cmd_rsp_data_st *rsp_data)
{
    ...
    uint32_t ret = EHSM_ERR_SW_SUCCESS;
    const mb_cmd_close_debug_st *cmd_data = NULL;

    if ((req_data == NULL) || (rsp_data == NULL)) {
        ret = EHSM_ERR_PARAM_ERROR;
    } else {
        cmd_data = (const mb_cmd_close_debug_st *)(req_data);
        // Check the operation life cycle is allowed
        if (debug_auth_check_lifecycle_by_type(cmd_data->type) != EHSM_ERR_SW_SUCCESS) {
            ret = EHSM_ERR_EHSM_LIFECYCLE_LIMIT;
        } else {
            // if challenge type is soc need to read the debug bit map
            if (MB_EHSM_CHALLENGE_TYPE_SOC_DEBUG == cmd_data->type) {
                if (cmd_data->soc_dbg_bitmap_size != 5U) {
                    ret = EHSM_ERR_PARAM_ERROR;
                } else {
                    ret = mmap_read_remote_data(soc_dbg_bitmap, cmd_data->soc_dbg_bitmap_addr, 5U * sizeof(uint32_t));
                }
            }
        }
    }
    ...
    if (EHSM_ERR_SW_SUCCESS == ret) {
        ...
        } else if (cmd_data->type == MB_EHSM_CHALLENGE_TYPE_SOC_DEBUG) {
            ret = dbgauth_switch_soc_debug_port(SOC_DBG_PORT_CLOSE_MODE, soc_dbg_bitmap);
...
/**
 *   @brief      Open or Close the soc debug port
 *
 *   @param [in] req_data
 *   @param [in] rsp_data
 *
...
static uint32_t dbgauth_switch_soc_debug_port(soc_dbg_switch_mode_e mode, const uint32_t soc_dbg_bitmap[5])
{
    ...
    if ((mode > SOC_DBG_PORT_CLOSE_MODE) || (soc_dbg_bitmap == NULL)) {
        ret = EHSM_ERR_PARAM_ERROR;
    } else {
        if (SOC_DBG_PORT_OPEN_MODE == mode) {
            /*  Soc debug bit map size is 5 words.
                ...
                the last word only the lowest bit is valid(indicate whether 129 bits of the hardware debugging port are
               enabled).
            */
            sysreg_enable_soc_dbg_ext(0, soc_dbg_bitmap[0]);
            sysreg_enable_soc_dbg_ext(1, soc_dbg_bitmap[1]);
            sysreg_enable_soc_dbg_ext(2, soc_dbg_bitmap[2]);
            sysreg_enable_soc_dbg_ext(3, soc_dbg_bitmap[3]);
            ...
            /*  Soc debug bit map size is 5 words.
                the first 4 words indicate whether to close the corresponding hardware debug port per bit.
                the last word only the lowest bit is valid(indicate whether 129 bits of the hardware debugging port are
               disabled).
            */
...
uint32_t dbgauth_ehsm_debug_auth(uint8_t challenge_type, ehsm_debug_auth_st *debug_auth_st)
{
    ...
    if (FID_EQ(ret, EHSM_ERR_SW_SUCCESS)) {
        ...
        if (FID_EQ(MB_EHSM_CHALLENGE_TYPE_EHSM_DEBUG, challenge_type)) {
            ...
            ret = dbgauth_switch_soc_debug_port(SOC_DBG_PORT_OPEN_MODE, debug_auth_st->soc_dbg_bitmap);
        }
    }
...
uint32_t dbgauth_get_challenge(uint8_t type, uint8_t *challenge_buf)
{
    ...
    if (EHSM_ERR_SW_SUCCESS == ret) {
        ...
        if (EHSM_ERR_SW_SUCCESS == ret) {
            ...
            if (EHSM_ERR_SW_SUCCESS == ret) {
                util_memcpy(challenge_buf, out_data, EHSM_DEBUG_CHALLENGE_SIZE);
                set_challenge_st(type, out_data);
            }
        }
    }
...
uint32_t dbgauth_close_debug(uint8_t type)
{
    ...
    if (debug_auth_check_lifecycle_by_type(type) != EHSM_ERR_SW_SUCCESS) {
        ...
        if (type == MB_EHSM_CHALLENGE_TYPE_EHSM_DEBUG) {
            ...
            util_memset(soc_dbg_bitmap, 0xFF, sizeof(soc_dbg_bitmap));
            ret = dbgauth_switch_soc_debug_port(SOC_DBG_PORT_CLOSE_MODE, soc_dbg_bitmap);
        } else if (type == MB_EHSM_CHALLENGE_TYPE_USER_AUTH) {
            g_user_auth_result = false;
        } else {
