#include "../../crypto_include/hash_hmac/hmac_sha384.h"
 ...
 * parameters:
 *     ctx ------------------------ input, HMAC_SHA384_CTX context pointer
 *     key ------------------------ input, key
 ...
 * parameters:
 *     ctx ------------------------ input, HMAC_SHA384_DMA_CTX context pointer
 *     key ------------------------ input, key
 *     sp_key_idx ----------------- input, index of secure port key
...
/* function: dma hmac-sha384 message update done, get the hmac
 * parameters:
 *     ctx ------------------------ input, HMAC_SHA384_DMA_CTX context pointer
...
/* function: dma hmac-sha384 input key and message, get the hmac
 * parameters:
 *     key ------------------------ input, key
 *     sp_key_idx ----------------- input, index of secure port key
 *     key_bytes ------------------ input, key byte length
...
{
    return hmac_dma(HASH_SHA384, key, sp_key_idx, key_bytes, msg_h, msg_l, msg_bytes, mac_h, mac_l, 
            callback);
}
#else
...
{
    return hmac_dma_node_steps(HASH_SHA384, key, sp_key_idx, key_bytes, node, node_num, mac_h, mac_l, 
            callback);
}
#else
uint32_t hmac_sha384_dma_node_steps(const uint8_t *key, uint16_t sp_key_idx, uint32_t key_bytes, 
