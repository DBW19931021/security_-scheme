...
/* function: input whole message and get its sha1 digest(node style)
 * parameters:
 ...
 *     msg_bytes ------------------ input, byte length of the input message, must be a multiple of sha1
...
{
    return hash_dma(HASH_SHA1, msg_h, msg_l, msg_bytes, digest_h, digest_l, callback);
}
#else
...
/* function: input whole message and get its sha1 digest(dma node style)
 * parameters:
 *     node ----------------------- input, message node pointer
 *     node_num ------------------- input, number of hash nodes, i.e. number of message segments.
 *     digest --------------------- output, sha1 digest, 20 bytes
...
{
    return hash_dma_node_steps(HASH_SHA1, node, node_num, digest_h, digest_l, callback);
}
#else
