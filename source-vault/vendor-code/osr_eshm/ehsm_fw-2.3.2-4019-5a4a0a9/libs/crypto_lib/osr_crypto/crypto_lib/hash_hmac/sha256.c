 ...
 *     1. please make sure the digest buffer is sufficient
 */
uint32_t sha256(const uint8_t *msg, uint32_t msg_bytes, uint8_t *digest)
{
...
/* function: input whole message and get its sha256 digest(node style)
 * parameters:
 ...
 */
uint32_t sha256_node_steps(const hash_node_st *node, uint32_t node_num, uint8_t *digest)
{
 ...
 *     msg_bytes ------------------ input, byte length of the input message, must be a multiple of sha256
...
uint32_t sha256_dma_update_blocks(SHA256_DMA_CTX *ctx, uint32_t msg_h, uint32_t msg_l, uint32_t msg_bytes)
{
    return hash_dma_update_blocks(ctx, msg_h, msg_l, msg_bytes);
}
#else
uint32_t sha256_dma_update_blocks(SHA256_DMA_CTX *ctx, uint32_t *msg, uint32_t msg_bytes)
{
 ...
 */
#ifdef CONFIG_HASH_SUPPORT_DMA_HIGH_LOW_ADDRESS
uint32_t sha256_dma_final(SHA256_DMA_CTX *ctx, uint32_t remainder_msg_h, uint32_t remainder_msg_l, 
 ...
 */
#ifdef CONFIG_HASH_SUPPORT_DMA_HIGH_LOW_ADDRESS
uint32_t sha256_dma(uint32_t msg_h, uint32_t msg_l, uint32_t msg_bytes, uint32_t digest_h, uint32_t digest_l, 
        HASH_CALLBACK callback)
{
    return hash_dma(HASH_SHA256, msg_h, msg_l, msg_bytes, digest_h, digest_l, callback);
}
#else
...
/* function: input whole message and get its sha256 digest(dma node style)
 * parameters:
 *     node ----------------------- input, message node pointer
 *     node_num ------------------- input, number of hash nodes, i.e. number of message segments.
 *     digest --------------------- output, sha256 digest, 32 bytes
 ...
 */
#ifdef CONFIG_HASH_SUPPORT_DMA_HIGH_LOW_ADDRESS
uint32_t sha256_dma_node_steps(const hash_dma_node_st *node, uint32_t node_num, uint32_t digest_h, 
        uint32_t digest_l, HASH_CALLBACK callback)
{
    return hash_dma_node_steps(HASH_SHA256, node, node_num, digest_h, digest_l, callback);
}
#else
uint32_t sha256_dma_node_steps(const hash_dma_node_st *node, uint32_t node_num, uint32_t *digest, 
        HASH_CALLBACK callback)
{
