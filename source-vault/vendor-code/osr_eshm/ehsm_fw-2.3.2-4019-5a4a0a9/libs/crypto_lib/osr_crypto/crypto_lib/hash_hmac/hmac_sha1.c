 ...
 * caution:
 */
uint32_t hmac_sha1_init(HMAC_SHA1_CTX *ctx, const uint8_t *key, uint16_t sp_key_idx, uint32_t key_bytes)
{
 ...
 */
uint32_t hmac_sha1(const uint8_t *key, uint16_t sp_key_idx, uint32_t key_bytes, const  uint8_t *msg, 
 ...
 */
uint32_t hmac_sha1_node_steps(const uint8_t *key, uint16_t sp_key_idx, uint32_t key_bytes, 
...
uint32_t hmac_sha1_dma_update_blocks(HMAC_SHA1_DMA_CTX *ctx, uint32_t msg_h, uint32_t msg_l, uint32_t msg_bytes)
{
    return hmac_dma_update_blocks(ctx, msg_h, msg_l, msg_bytes);
}
#else
uint32_t hmac_sha1_dma_update_blocks(HMAC_SHA1_DMA_CTX *ctx, uint32_t *msg, uint32_t msg_bytes)
{
 ...
 */
#ifdef CONFIG_HASH_SUPPORT_DMA_HIGH_LOW_ADDRESS
uint32_t hmac_sha1_dma_final(HMAC_SHA1_DMA_CTX *ctx, uint32_t remainder_msg_h, uint32_t remainder_msg_l, 
...
#else
uint32_t hmac_sha1_dma_final(HMAC_SHA1_DMA_CTX *ctx, uint32_t *remainder_msg, uint32_t remainder_bytes, 
        uint32_t *mac)
{
 ...
 */
#ifdef CONFIG_HASH_SUPPORT_DMA_HIGH_LOW_ADDRESS
uint32_t hmac_sha1_dma(const uint8_t *key, uint16_t sp_key_idx, uint32_t key_bytes, uint32_t msg_h, uint32_t msg_l, 
        uint32_t msg_bytes, uint32_t mac_h, uint32_t mac_l, HASH_CALLBACK callback)
{
    return hmac_dma(HASH_SHA1, key, sp_key_idx, key_bytes, msg_h, msg_l, msg_bytes, mac_h, mac_l, callback);
}
#else
uint32_t hmac_sha1_dma(const uint8_t *key, uint16_t sp_key_idx, uint32_t key_bytes, uint32_t *msg, 
 ...
 */
#ifdef CONFIG_HASH_SUPPORT_DMA_HIGH_LOW_ADDRESS
uint32_t hmac_sha1_dma_node_steps(const uint8_t *key, uint16_t sp_key_idx, uint32_t key_bytes, 
        const  hash_dma_node_st *node, uint32_t node_num, uint32_t mac_h, uint32_t mac_l, HASH_CALLBACK callback)
{
    return hmac_dma_node_steps(HASH_SHA1, key, sp_key_idx, key_bytes, node, node_num, mac_h, mac_l,  
            callback);
}
#else
uint32_t hmac_sha1_dma_node_steps(const uint8_t *key, uint16_t sp_key_idx, uint32_t key_bytes, 
