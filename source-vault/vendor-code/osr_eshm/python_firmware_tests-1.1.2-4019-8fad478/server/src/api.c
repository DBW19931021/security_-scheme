...
static inline uint32_t ehsm_wait_until_responed(ehsm_ctx_intl_st *ctx_intl)
{
...
static void ehsm_mb_int(uint32_t channel, raddr_t packet_addr)
{
    ...
    case MB_CMD_ID_TBBR_IMG_VERIFY:
        if (ctx_intl->result1 != NULL) {
            *((uint32_t *)ctx_intl->result1) = ctx_intl->rsp_buf.data[0];
        }
        if (ctx_intl->result2 != NULL) {
...
uint32_t ehsm_send_cmd(ehsm_ctx_intl_st *ctx_intl)
{
    ...
    case EHSM_DRV_MODE_INTERRUPT:
        if (!ctx_intl->async) {
            ret = ehsm_wait_until_responed(ctx_intl);
            if (ret != EHSM_OK) {
                return ret;
            }
...
uint32_t ehsm_aead_update(ehsm_ctx_st *ctx, const uint8_t *input, uint32_t input_size, uint8_t *output)
{
    ...
    if (ctx_intl->cmd_buf.cmd_id == MB_CMD_ID_AEAD_GCM) {
        ...
        mb_cmd_aead_gcm_st *ccm_cmd = (mb_cmd_aead_gcm_st *)&ctx_intl->cmd_buf;
    ...
    uint32_t *sign_size, bool_t *verify_result)
{
    ...
    ehsm_ctx_intl_st *ctx_intl = (ehsm_ctx_intl_st *)ctx;
    if (gen_sig) {
        ctx_intl->result2 = (void *)sign_size;
    } else {
        ctx_intl->result1 = (void *)verify_result;
    }

    mb_cmd_pqc_dsa_st *cmd = (mb_cmd_pqc_dsa_st *)&ctx_intl->cmd_buf;
    /* 初始化 PQC DSA Mailbox 命令结构，ehsm_ctx_reset 已将所有字段清零 */
    ehsm_set_cmd_id((ehsm_mb_cmd_st *)cmd, MB_CMD_ID_PQC_DSA);
    cmd->key_type = use_plain_key ? MB_PLAIN_KEY : MB_KEY_HANDLE;
    cmd->direction = gen_sig ? MB_SIG_GEN : MB_SIG_VRY;
    cmd->sign_algo = (uint8_t)sign_algo;
    cmd->sign_mode = (uint8_t)sign_mode;
    cmd->is_det = is_det ? 1U : 0U;

    if (MB_PQC_PURE_DSA_WITH_MSG != sign_mode) {
        cmd->hash_algo = (uint8_t)hash_algo;
    }
    ...
    cmd->ctx_str_addr = ehsm_port_addr_to_raddr(pqc_ctx_str);
    cmd->ctx_str_size = pqc_ctx_str_size;
    cmd->input_addr = ehsm_port_addr_to_raddr(input);
    cmd->input_size = input_size;
    cmd->sign_addr = (raddr_t)sig_addr;
    cmd->sign_size = *sign_size;
    ...
    uint32_t *ss_key_handle, EHSM_SHM uint8_t *ss_key, uint32_t ss_key_size)
{
    ...
    ehsm_ctx_reset(ctx);
    ehsm_ctx_intl_st *ctx_intl = (ehsm_ctx_intl_st *)ctx;
    if (EHSM_PQC_OUT_KEY_HANDLE == ss_out_type) {
        ctx_intl->result1 = (void *)ss_key_handle;
    }

    if (is_encaps) {
        ctx_intl->result2 = (void *)cipher_key_data_size;
    }

    mb_cmd_pqc_ml_kem_st *cmd = (mb_cmd_pqc_ml_kem_st *)&ctx_intl->cmd_buf;
    // 默认字段都设为了0，这里不会重复设置
    ehsm_set_cmd_id((ehsm_mb_cmd_st *)cmd, MB_CMD_ID_PQC_ML_KEM);
    cmd->parent_key_type = (uint8_t)(use_plain_parent_key ? MB_PLAIN_KEY : MB_KEY_HANDLE);
    cmd->direction = (uint8_t)(is_encaps ? MB_ENCAPS : MB_DECAPS);
    cmd->ss_key_out_type = (uint8_t)ss_out_type;

    if (EHSM_PQC_OUT_KEY_HANDLE == ss_out_type) {
        cmd->ss_key_algo_id = (uint8_t)ss_key_type;
        cmd->ss_key_handle = *ss_key_handle;
        cmd->ss_key_permit = ss_privilege;
    } else {
        cmd->ss_key_addr = ehsm_port_addr_to_raddr(ss_key);
        cmd->ss_key_size = ss_key_size;
    }

    if (use_plain_parent_key) {
        cmd->parent_key_addr = ehsm_port_addr_to_raddr(parent_key);
    } else {
        cmd->parent_key_handle = parent_key_handle;
    }

    cmd->cipher_key_data_addr = ehsm_port_addr_to_raddr(cipher_key_data);
    cmd->cipher_key_data_size = *cipher_key_data_size;
...
uint32_t ehsm_bl_verify_image_discrete(EHSM_SHM ehsm_ctx_st *ctx, EHSM_SHM const uint8_t *image, uint32_t image_size,
    EHSM_SHM const uint8_t *code, bool_t only_copy_code, bool_t check_version, bool_t boot, EHSM_SHM uint8_t *image_out)
{
    uint32_t ret = ehsm_check_ctx(ctx);
    if (ret != EHSM_OK) {
        return ret;
    ...
    cmd->check_version = check_version ? 1 : 0;
    cmd->image_out_addr = ehsm_port_addr_to_raddr(image_out);
    cmd->code_addr = ehsm_port_addr_to_raddr(code);
    cmd->only_copy_code = only_copy_code ? MB_BL_VERIFY_IMAGE_ONLY_COPY_CODE_YES : MB_BL_VERIFY_IMAGE_ONLY_COPY_CODE_NO;

    return ehsm_send_cmd(ctx_intl);
}
// #% #endif // CONFIG_BL_CUSTOM_ID != 0xb001

STATIC_ASSERT(sizeof(mb_cmd_bl_inject_err_st) == sizeof(mb_cmd_inject_err_st));
STATIC_ASSERT(MB_CMD_ID_BL_INJECT_ERR == MB_CMD_ID_INJECT_ERR);
...
uint32_t ehsm_inject_error(EHSM_SHM ehsm_ctx_st *ctx, const ehsm_inject_error_st *values)
{
    ...
    mb_cmd_bl_inject_err_st *cmd = (mb_cmd_bl_inject_err_st *)&ctx_intl->cmd_buf;
    ehsm_set_cmd_id((ehsm_mb_cmd_st *)cmd, MB_CMD_ID_BL_INJECT_ERR);
    if (values) {
        cmd->fw_trig[0] = values->fw_trig[0];
        cmd->fw_trig[1] = values->fw_trig[1];
        cmd->hw_trig[0] = values->hw_trig[0];
        cmd->hw_trig[1] = values->hw_trig[1];
        cmd->alarm_trig[0] = values->alarm_trig[0];
        cmd->alarm_trig[1] = values->alarm_trig[1];
    }
    ...
    uint32_t *non_trusted_fw_nv_ctr_in_otp_cert)
{
    ...
    mb_cmd_tbbr_img_verify_st *cmd = (mb_cmd_tbbr_img_verify_st *)&ctx_intl->cmd_buf;
    ehsm_set_cmd_id((ehsm_mb_cmd_st *)cmd, MB_CMD_ID_TBBR_IMG_VERIFY);
    cmd->img_size = img_size;
    cmd->img_addr = ehsm_port_addr_to_raddr(img_data);
    cmd->trusted_fw_nv_ctr = trusted_fw_nv_ctr_in_otp;
    cmd->non_trusted_fw_nv_ctr = non_trusted_fw_nv_ctr_in_otp;
    ctx_intl->result1 = (void *)trusted_fw_nv_ctr_in_cert;
    ctx_intl->result2 = (void *)non_trusted_fw_nv_ctr_in_otp_cert;
...
uint32_t ehsm_bl_set_hsm_freq(EHSM_SHM ehsm_ctx_st *ctx, uint32_t freq)
{
    uint32_t ret = ehsm_check_ctx(ctx);
    if (ret != EHSM_OK) {
        return ret;
    ...
    mb_cmd_bl_set_hsm_freq_st *cmd = (mb_cmd_bl_set_hsm_freq_st *)&ctx_intl->cmd_buf;
    ehsm_set_cmd_id((ehsm_mb_cmd_st *)cmd, MB_CMD_ID_BL_SET_HSM_FREQ);
    cmd->hsm_freq = freq;
...
uint32_t ehsm_bl_get_socid(EHSM_SHM ehsm_ctx_st *ctx, EHSM_SHM uint8_t *socid)
{
    ...
    mb_cmd_bl_get_socid_st *cmd = (mb_cmd_bl_get_socid_st *)&ctx_intl->cmd_buf;
    ehsm_set_cmd_id((ehsm_mb_cmd_st *)cmd, MB_CMD_ID_BL_GET_SOCID);
    cmd->socid_addr = ehsm_port_addr_to_raddr(socid);
