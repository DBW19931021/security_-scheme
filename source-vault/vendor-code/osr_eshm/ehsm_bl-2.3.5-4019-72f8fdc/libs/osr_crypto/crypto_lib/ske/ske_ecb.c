...
#include "../../crypto_include/ske/ske_ecb.h"
...
/* function: ske ecb init config(CPU style)
 * parameters:
...
{
    return ske_init(ctx, alg, SKE_MODE_ECB, crypto, key, sp_key_idx, NULL, padding);
}
...
/* function: ske ecb encryption or decryption for input including tail(CPU style)
 * parameters:
 *     ctx ------------------------ input, ske_ctx_st context pointer
 *     in ------------------------- input, plaintext or ciphertext
 *     out ------------------------ output, ciphertext or plaintext
...
/* function: ske ecb encrypting or decrypting(CPU style, one-off style)
 * parameters:
 *     alg ------------------------ input, ske algorithm
...
{
    return ske_crypto(alg, SKE_MODE_ECB, crypto, key, sp_key_idx, NULL, padding, in, out, in_bytes, out_bytes);
}
...
uint32_t ske_dma_ecb_init(ske_ctx_st *ctx, ske_alg_e alg, ske_crypto_e crypto, const uint8_t *key, uint16_t sp_key_idx,
        ske_padding_e padding)
{
    return ske_dma_init(ctx, alg, SKE_MODE_ECB, crypto, key, sp_key_idx, NULL, padding);
}
...
{
    return ske_dma_crypto(alg, SKE_MODE_ECB, crypto, key, sp_key_idx, NULL, padding, in_h, in_l, out_h, out_l,
...
uint32_t ske_dma_ecb_crypto(ske_alg_e alg, ske_crypto_e crypto, const uint8_t *key, uint16_t sp_key_idx,
        ...
        SKE_CALLBACK callback)
{
    return ske_dma_crypto(alg, SKE_MODE_ECB, crypto, key, sp_key_idx, NULL, padding, in, out, 
            in_bytes, out_bytes, callback);
}
#endif
