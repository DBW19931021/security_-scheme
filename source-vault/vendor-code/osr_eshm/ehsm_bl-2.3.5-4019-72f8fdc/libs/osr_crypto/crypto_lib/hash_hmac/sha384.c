#include "../../crypto_include/hash_hmac/sha384.h"
 ...
 * parameters:
 *     ctx ------------------------ input, SHA384_CTX context pointer
 * return: HASH_SUCCESS(success), other(error)
 * caution:
 */
uint32_t sha384_init(SHA384_CTX *ctx)
{
 ...
 */
uint32_t sha384_update(SHA384_CTX *ctx, const uint8_t *msg, uint32_t msg_bytes)
{
 ...
 */
uint32_t sha384_final(SHA384_CTX *ctx, uint8_t *digest)
{
 ...
 *     1. please make sure the digest buffer is sufficient
 */
uint32_t sha384(const uint8_t *msg, uint32_t msg_bytes, uint8_t *digest)
{
...
/* function: input whole message and get its sha384 digest(node style)
 * parameters:
 ...
 */
uint32_t sha384_node_steps(const hash_node_st *node, uint32_t node_num, uint8_t *digest)
{
 ...
 * parameters:
 *     ctx ------------------------ input, SHA384_DMA_CTX context pointer
 *     callback ------------------- input, callback function pointer
 * return: HASH_SUCCESS(success), other(error)
 * caution:
 */
uint32_t sha384_dma_init(SHA384_DMA_CTX *ctx, HASH_CALLBACK callback)
{
 ...
 *     msg_bytes ------------------ input, byte length of the input message, must be a multiple of sha384
...
uint32_t sha384_dma_update_blocks(SHA384_DMA_CTX *ctx, uint32_t msg_h, uint32_t msg_l, uint32_t msg_bytes)
{
    return hash_dma_update_blocks(ctx, msg_h, msg_l, msg_bytes);
}
#else
uint32_t sha384_dma_update_blocks(SHA384_DMA_CTX *ctx, uint32_t *msg, uint32_t msg_bytes)
{
 ...
 */
#ifdef CONFIG_HASH_SUPPORT_DMA_HIGH_LOW_ADDRESS
uint32_t sha384_dma_final(SHA384_DMA_CTX *ctx, uint32_t remainder_msg_h, uint32_t remainder_msg_l, 
...
#else
uint32_t sha384_dma_final(SHA384_DMA_CTX *ctx, uint32_t *remainder_msg, uint32_t remainder_bytes, uint32_t *digest)
{
 ...
 */
#ifdef CONFIG_HASH_SUPPORT_DMA_HIGH_LOW_ADDRESS
uint32_t sha384_dma(uint32_t msg_h, uint32_t msg_l, uint32_t msg_bytes, uint32_t digest_h, uint32_t digest_l, 
        HASH_CALLBACK callback)
{
    return hash_dma(HASH_SHA384, msg_h, msg_l, msg_bytes, digest_h, digest_l, callback);
}
#else
uint32_t sha384_dma(uint32_t *msg, uint32_t msg_bytes, uint32_t *digest, HASH_CALLBACK callback)
{
...
/* function: input whole message and get its sha384 digest(dma node style)
 * parameters:
 *     node ----------------------- input, message node pointer
 *     node_num ------------------- input, number of hash nodes, i.e. number of message segments.
 *     digest --------------------- output, sha384 digest, 48 bytes
 ...
 */
#ifdef CONFIG_HASH_SUPPORT_DMA_HIGH_LOW_ADDRESS
uint32_t sha384_dma_node_steps(const hash_dma_node_st *node, uint32_t node_num, uint32_t digest_h, 
        uint32_t digest_l, HASH_CALLBACK callback)
{
    return hash_dma_node_steps(HASH_SHA384, node, node_num, digest_h, digest_l, callback);
}
#else
uint32_t sha384_dma_node_steps(const hash_dma_node_st *node, uint32_t node_num, uint32_t *digest,
        HASH_CALLBACK callback)
{
