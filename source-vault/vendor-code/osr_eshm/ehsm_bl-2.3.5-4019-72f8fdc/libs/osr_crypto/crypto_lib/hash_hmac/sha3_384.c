#include "../../crypto_include/hash_hmac/sha3_384.h"
 ...
 * parameters:
 *     ctx ------------------------ input, SHA3_384_CTX context pointer
...
/* function: input whole message and get its sha3_384 digest(node style)
 * parameters:
...
{
    return hash_node_steps(HASH_SHA3_384, node, node_num, digest);
}
#endif
 ...
 * parameters:
 *     ctx ------------------------ input, SHA3_384_DMA_CTX context pointer
 ...
 *     msg_bytes ------------------ input, byte length of the input message, must be a multiple of sha3_384
 *                                  block byte length(104)
 * return: HASH_SUCCESS(success), other(error)
 * caution:
...
{
    return hash_dma(HASH_SHA3_384, msg_h, msg_l, msg_bytes, digest_h, digest_l, callback);
}
#else
...
/* function: input whole message and get its sha3_384 digest(dma node style)
 * parameters:
 *     node ----------------------- input, message node pointer
 *     node_num ------------------- input, number of hash nodes, i.e. number of message segments.
 *     digest --------------------- output, sha3_384 digest, 48 bytes
...
{
    return hash_dma_node_steps(HASH_SHA3_384, node, node_num, digest_h, digest_l, callback);
}
#else
uint32_t sha3_384_dma_node_steps(const hash_dma_node_st *node, uint32_t node_num, uint32_t *digest,
        HASH_CALLBACK callback)
{
    return hash_dma_node_steps(HASH_SHA3_384, node, node_num, digest, callback);
}
#endif
#endif
