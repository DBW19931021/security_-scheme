...
/**
 * @brief Bootloader/Firmware的版本号结构体。
 */
typedef struct {
...
typedef enum {
    ...
    EHSM_KEY_PART_PRIVATE_KEY
    = 2, /**< 密钥数据结构体中仅包含私钥（与 EHSM_KEY_PART_SYMM_KEY 共享同一值，这是有意设计） */
    EHSM_KEY_PART_SYMM_KEY
    = 2, /**< 密钥数据结构体包含的是对称密钥（与 EHSM_KEY_PART_PRIVATE_KEY 共享同一值，这是有意设计） */
...
typedef enum {
    EHSM_KEY_LEVEL_1 = 1, /**< 此OTP密钥由CHIP ROOT KEY加密，仅支持 `TEST_MODE` 和 `DEVELOP_MODE` */
    EHSM_KEY_LEVEL_2
 ...
 * @retval 其它值 任务发生错误
 */
static inline uint32_t ehsm_sm2_sign_onepass_gen_with_digest(EHSM_SHM ehsm_ctx_st *ctx, uint32_t key_handle,
    EHSM_SHM const uint8_t *digest, uint32_t digest_size, EHSM_SHM uint8_t *sig, uint32_t sig_size)
{
    return ehsm_sm2_sign_onepass_ex(ctx, /*use_plain_key=*/false, key_handle, /*key=*/NULL, /*gen_sig=*/true,
 ...
 * @retval 其它值 任务发生错误
 */
static inline uint32_t ehsm_sm2_sign_onepass_verify_with_digest(EHSM_SHM ehsm_ctx_st *ctx, uint32_t key_handle,
    EHSM_SHM const uint8_t *digest, uint32_t digest_size, EHSM_SHM const uint8_t *sig, uint32_t sig_size,
    bool_t *verify_result)
{
    return ehsm_sm2_sign_onepass_ex(ctx, /*use_plain_key=*/false, key_handle, /*key=*/NULL, /*gen_sig=*/false,
