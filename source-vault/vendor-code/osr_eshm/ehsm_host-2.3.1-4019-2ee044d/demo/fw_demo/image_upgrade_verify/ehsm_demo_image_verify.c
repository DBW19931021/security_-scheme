...
#include "ehsm_demo_image_verify.h"
...
typedef struct {
    ...
    const uint8_t *image;
    uint32_t image_size;
} demo_image_verify_std_st;

/* clang-format off */

static demo_image_verify_std_st s_image_verify_std_data[2] = {
    {
 ...
 * @brief SoC 镜像校验的演示。
 *
 * @param[in] std_data SoC 镜像校验的标准演示数据，详见 @ref demo_image_verify_std_st
 *
 * @note Firmware 下仅支持 SoC 镜像的校验。
 *
 */
static void demo_image_verify(const demo_image_verify_std_st *std_data)
{
    uint32_t ret;
    ...
    /* 对于 SoC 镜像，无跳转的说法，因此设为 false，且镜像若为加密镜像，需要设置 image_out 作为解密后的镜像输出地址 */
    ret = ehsm_verify_image(ctx, image, image_size, std_data->check_version, false, image_out);
...
/**
 * @brief 校验 SoC 镜像的演示入口函数。
 */
void ehsm_demo_image_verify_entry(void)
{
    uint32_t i;
    demo_image_verify_std_st *std_data = s_image_verify_std_data;
    uint32_t cnt = sizeof(s_image_verify_std_data) / sizeof(demo_image_verify_std_st);
    ehsm_port_printf("\r\n\r\n\r\n\r\n==================== eHSM demo for image verification starts. "
                     "====================\r\n\r\n\r\n\r\n");

    for (i = 0U; i < cnt; i++) {
        ehsm_port_printf("demo for std_data[%d].\r\n", i);
        demo_image_verify(&std_data[i]);
    }
