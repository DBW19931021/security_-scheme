...
1. **BootROM early 阶段**：上电或复位后，BootROM 先做最小平台初始化，只读取启动所需的 `secure_boot_enable`、lifecycle、strap 和 control field 等配置。该阶段不信任 Host 输入，也不进入复杂更新逻辑。
2. **等待 eHSM ready**：BootROM 必须等待 eHSM 完成 ROM/BL/FW 自检、eFuse 装载和生命周期恢复。eHSM 未 ready 或 mailbox 超时都属于受控失败，BootROM 不允许绕过 eHSM 继续执行 FMC。
3. **读取固定 FMC 包**：BootROM 只从 NOR Flash 的固定 FMC 分区读取 `FMC(SEC1)` secure package。首版不选择 slot，不解析复杂 slot metadata，也不执行 boot confirm/fallback 状态机；FMC 不由 Host 通过 SoC 启动链路下发。
4. **FMC verify/decrypt 裁决**：BootROM 调用 eHSM `VERIFY_FMC(package_addr, output_buf)`，由 eHSM 完成 header、key、revoke、rollback、signature 和强制解密检查。正式路径中 FMC 必须签名 + 加密，验证或解密失败时 BootROM 停止跳转，可选记录诊断状态并等待复位或 OOB 刷机恢复。
5. **FMC manifest 与跳转**：eHSM 返回 PASS 后，BootROM 对 NGU protected manifest 做最小策略检查，包括 load 地址、entry 地址、lifecycle 约束和 measurement 记录。检查通过后，BootROM 将控制权交给 FMC；至此启动链进入 `BootROM -> FMC(SEC1, fixed NOR Flash partition)`。
6. **FMC 建立 Host 通道并验证 GSP**：FMC 完成基础初始化并建立 Host PCIe 通道。Host 只负责把 `GSP(SEC2)` protected package 传输到 staging buffer，不拥有 release 权；正式路径中 GSP 也必须签名 + 加密，FMC 调 eHSM `VERIFY_IMAGE(GSP)` 完成验签、解密、rollback、revoke 和 manifest policy 检查，PASS 后才 release GSP。
7. **GSP 验证 Runtime 固件**：GSP 接管运行期安全控制面后，继续接收 Host 下发的 PM/RAS/Codec/Other Runtime FW package，并调用 eHSM 完成 `VERIFY_IMAGE`、measurement 和 release policy 检查。Runtime 固件默认签名 + 加密，只有产品白名单允许的非敏感固件才可采用 signature-only。
8. **失败处理原则**：任一关键镜像验证失败都不允许降级为继续执行。GSP 验证失败时 FMC 向 Host 返回错误并等待重新下发；Runtime 验证失败时 GSP 拒绝 release 对应微核；FMC 验证失败时 BootROM 留在 halt / wait reset 状态。
9. **OOB 刷机恢复关系**：FMC 验证失败后的恢复不由 BootROM 自动 fallback 完成，而是等待 Host/BMC 触发 OOB MCU。OOB MCU 通过自身独立信任链检查 recovery flash command + package，经 QSPI 重刷 NOR Flash 固定 FMC 分区；重刷后的 FMC 仍需在下一次启动中由 BootROM + eHSM 完成最终 verify/decrypt 裁决。
...
```mermaid
flowchart TD
    A[升级授权方签发 key rotation capsule] --> B[SEC/GSP 校验更新 capsule<br/>或 OOB MCU 校验刷机 capsule]
    B --> C[安装 new signer / FW_KEK policy 为 PENDING]
    C --> D[生成使用 new policy 的 FMC package<br/>供 OOB 刷机或下一次启动验证]
    D --> E[确认 OOB MCU 可通过 QSPI 重刷固定 FMC 分区]
    E --> F{新 FMC 启动验证通过?}
...
1. 升级授权方签发 key rotation capsule。
2. SEC/GSP 或 OOB MCU 根据场景校验更新或刷机授权。
3. 新 signer/FW_KEK policy 先进入 PENDING，不立即撤销旧 key。
4. 确认 OOB 可用新策略重刷固定 FMC 分区。
5. 新 FMC 经 BootROM/eHSM 启动验证通过后，新 key 才 active，旧 key 再进入 deprecated/revoke 流程。
...
sequenceDiagram
    ...
    alt write check PASS
        ...
        alt eHSM PASS
            ...
            BR->>BR: halt; wait next OOB flashing
        end
    else write check FAIL
        OOB->>Tool: report failure; request resend/reflash
    end
```
