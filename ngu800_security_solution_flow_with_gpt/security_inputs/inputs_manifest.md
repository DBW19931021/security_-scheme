...
| `SRC-009` | `SRC-009 OSR eHSM 软件代码包 4019` | `security_inputs/sw/` |
| `SRC-010` | `SRC-010 eHSM4.0 ROM Patch 方案` | `security_inputs/ip_manuals/ehsm/eHSM4.0 Patch方案.pdf` |
...
| SRC-009 | `security_inputs/sw/` | OSR eHSM 软件代码包 4019 | OSR / eHSM owner | 2026-06-26 import / BL 2.3.5-4019, FW 2.3.2-4019, Host 2.3.1-4019 | `confirmed` | `implementation-authority` | eHSM BL/FW/Host API 已提供的安全服务、mailbox command ID / req / rsp 结构、Host driver 行为、secure boot verify/upgrade、OTP/key/counter、debug/lifecycle、外部 OTP/Flash driver API、image tool / OTP tool 对接 | 用户明确：后续 HSM 代码已经提供的安全服务原则上以该代码为准，安全方案需要适配该代码；若现有方案需要 eHSM 未提供服务，必须登记差异并通过 wrapper/customization/方案调整 CR 处理 | 可作为 `04_impl_design`、`10_full_design`、code rules、traceability 和后续代码开发的实现级事实源；随包 TRM 4019 文档可用于解释代码 ABI | 不自动覆盖 Root of Trust、Host trust boundary、SEC1/SEC2 sign+encrypt、eHSM first verifier 等已冻结架构原则；不把未经逐项核对的字段、offset、key ID 或工具 CLI 自动升级为系统级冻结项 | `SRC-006`, `SRC-007` 的实现级细化 / 4019 版本补充 | 本源优先用于“eHSM 已实际提供什么服务、字段和命令”；与旧 TRM 或方案正文冲突时，应按 CR-0018 做差异分析并同步方案 |
| SRC-010 | `security_inputs/ip_manuals/ehsm/eHSM4.0 Patch方案.pdf` | eHSM4.0 ROM Patch 方案 | OSR / eHSM owner |  | `confirmed` | `preferred` | eHSM ROM 指令 patch 机制、OTP patch 表、硬件 BOOT 加载 patch 信息、CPU/IROM 无感替换、patch enable/address/data、patch hit 行为 | eHSM ROM patch 机制作为硬件 BOOT + OTP 配置机制纳入安全约束；patch 表固化在 OTP，硬件 BOOT 在 CPU 出复位前加载到 patch 模块，patch 对 CPU 无感，不作为 Host/SEC 运行期热补丁入口 | 可作为 eHSM ROM errata / OSR ROM 代码修复机制、OTP 空间规划、制造灌装和验收流程输入 | 不自动授权运行期软件修改 ROM patch；不自动冻结 NGU 项目的最终 OTP offset、patch 行使能策略、工站烧录流程或验收脚本 |  | PDF 结论包括 OTP 明文 patch data、不需要 APB CFG IF、patch hit 屏蔽 IROM 访问、增加 patch enable；字段级集成仍需 RTL/eHSM owner 确认 |
...
| CF-009 | eHSM 服务能力事实源：旧 TRM/方案抽象 vs OSR 软件代码 4019 | `SRC-009`, `CR-0018` | `SRC-006`, `SRC-007`, `10_full_design.md`, `04_impl_design/*.md` 中未逐项对齐的旧描述 | 用户明确后续 HSM 已提供安全服务以代码为准；方案需要适配代码。旧 TRM 和设计正文仍可保留原则和历史说明，但实现级命令、结构、错误、工具行为必须逐项对齐 OSR 软件包 | constraints、baseline、boot、mailbox、FW package、key/cert、debug/lifecycle、manufacturing、code rules、traceability | `open-by-CR-0018 / sync-needed` |
| CF-010 | eHSM ROM patch 机制尚未纳入现有 OTP / boot / manufacturing 设计 | `SRC-010`, `CR-0018` | 现有 `C-EHSM-01`、`efuse_key_fw_header_design.md` 和 manufacturing 章节未覆盖 patch 表 | Patch 表固化在 OTP 并在 eHSM 初始化时由硬件 BOOT 加载，会影响 ROM 实际执行指令，应纳入 OTP 空间、制造灌装、验收和 attestation/审计影响分析 | efuse_otp、boot_chain、manufacturing_provisioning、lifecycle_debug、traceability | `open-by-CR-0018 / field-freeze-needed` |
...
| CHG-011 | 2026-06-27 | `SRC-009`, `SRC-010`, `CR-0018` | 用户新增 OSR eHSM 软件代码包和 eHSM4.0 ROM Patch 方案，并明确后续 HSM 已提供服务原则上以 OSR 代码为准，安全方案需要适配代码 | 将影响 source precedence、constraints、baseline、eHSM source-conformance、mailbox/API、FW package、key/counter/debug、patch OTP、manufacturing、code rules、traceability 和后续 full design 同步 | `high` | 本轮先登记输入源和约束；`10_full_design.md` 与 `04_impl_design` 的字段级适配需后续专项 CR |
| CHG-012 | 2026-07-02 | `CR-0019` / 用户明确指示 | 用户明确 vendor 提供的 eHSM BootROM、eHSM Firmware 和 Wing 工具链后续改动必须谨慎；改动前需要明确提醒，并说明原因、改动点、影响和验证；改动后需记录到相关文档和 OpenSpec | 将影响 FSP OpenSpec、constraints、code rules、traceability、change impact 和后续代码/工具链修改流程 | `high` | 本轮只登记流程强约束，不修改 vendor 源码或工具链状态；后续真实 vendor 变更必须逐项记录原因、影响、回退和验证 |
