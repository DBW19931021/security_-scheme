...
static const ehsm_ver_data_st g_ver_data
    = {
          .type = 0,
          .major = EHSM_BL_VER_MAJOR,
          .minor = EHSM_BL_VER_MINOR,
          .patch = EHSM_BL_VER_PATCH,
          .pre_release = EHSM_BL_VER_PRE_RELEASE,
      };
...
{
    util_memset(dest, (uint8_t)ch, size);
    return dest;
}
#endif
...
int main(void)
{
    ...
    // write it to CSR general register 0
    *((volatile uint32_t *)SYS_GEN_REG) = v;
