...
VER_OFFSET_IN_BINARY = 0x80  # see this offset in link.ld
...
def get_version_from_header(config_h: str) -> Tuple[int, int, int, int, str]:
    ...
    major = re.search(r"""#define\s+EHSM_BL_VER_MAJOR\b\s+(\d+)""", content).group(1)
    minor = re.search(r"""#define\s+EHSM_BL_VER_MINOR\b\s+(\d+)""", content).group(1)
    patch = re.search(r"""#define\s+EHSM_BL_VER_PATCH\b\s+(\d+)""", content).group(1)
    pre_release = re.search(
        r'''#define\s+EHSM_BL_VER_PRE_RELEASE\b\s+"(.*)"''', content
    ).group(1)

    # the first 0 is type of bootloader
    return (0, int(major), int(minor), int(patch), pre_release)
