...
static uint32_t otpkey_check_key_usage(const otp_key_attributes_st *attr, uint8_t algo_type, uint8_t key_level, uint32_t key_usage);
/***********************************************************************************************************************
 *  LOCAL FUNCTION
 **********************************************************************************************************************/
static uint32_t otpkey_get_keyid_map_idx(uint32_t logic_key_id, uint8_t *idx)
...
static uint32_t otpkey_gen_key_data(const otp_key_attributes_st *attr, uint8_t *out)
{
    ...
    {
        ret = kmu_get_keyid_data(attr->otp_slot_id, out, 32);
    }
    else
    {
...
uint32_t otpkey_aquire_key_attr(uint32_t key_id, otp_key_attributes_st *otp_key_attr)
{
    uint8_t idx = 0;
    uint32_t ret= EHSM_ERR_SW_SUCCESS;
