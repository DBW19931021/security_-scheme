...
/* function: ske cbc init config(CPU style)
 * parameters:
...
{
    return ske_init(ctx, alg, SKE_MODE_CBC, crypto, key, sp_key_idx, iv, padding);
}
...
/* function: ske cbc encryption or decryption for input including tail(CPU style)
 * parameters:
 *     ctx ------------------------ input, ske_ctx_st context pointer
 *     in ------------------------- input, plaintext or ciphertext
 *     out ------------------------ output, ciphertext or plaintext
 ...
 *     4. if without padding scheme, in_bytes must be a multiple of block byte
...
/* function: ske cbc encrypting or decrypting(CPU style, one-off style)
 * parameters:
 *     alg ------------------------ input, ske algorithm
 ...
 *        cover the input.
 *     4. in_bytes must be a multiple of block byte length, out_bytes will be the same as in_bytes.
 */
...
{
    return ske_crypto(alg, SKE_MODE_CBC, crypto, key, sp_key_idx, iv, padding, in, out, in_bytes, out_bytes);
}
...
uint32_t ske_dma_cbc_init(ske_ctx_st *ctx, ske_alg_e alg, ske_crypto_e crypto, const uint8_t *key, uint16_t sp_key_idx,
        const uint8_t *iv, ske_padding_e padding)
{
    return ske_dma_init(ctx, alg, SKE_MODE_CBC, crypto, key, sp_key_idx, iv, padding);
}
...
{
    return ske_dma_crypto(alg, SKE_MODE_CBC, crypto, key, sp_key_idx, iv, padding, in_h, in_l, out_h, out_l, 
...
uint32_t ske_dma_cbc_crypto(ske_alg_e alg, ske_crypto_e crypto, const uint8_t *key, uint16_t sp_key_idx, const uint8_t *iv,
        ...
        SKE_CALLBACK callback)
{
    return ske_dma_crypto(alg, SKE_MODE_CBC, crypto, key, sp_key_idx, iv, padding, in, out, in_bytes, 
            out_bytes, callback);
}
#endif
